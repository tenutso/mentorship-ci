...
const categoryOptions = (props: { categories: any[] }) => props.categories.map(c => ({ label: c.name, value: c.id }))
const countryOptions = (props: { countries: any[] }) => props.countries.map(c => ({ label: c.name, value: c.id }))
...
<template>
  <UContainer as="section" class="py-12 border-b border-gray-200 dark:border-gray-800 relative">
    ...
    <UTabs :items="tabItems" :ui="{ list: { width: 'w-auto' } }">
      <template #mentee>
        <div class="pt-8">
      ...
      <template #mentor>
        <div class="pt-8">
          <h1 class="text-4xl md:text-5xl lg:w-3/S font-bold mb-2 text-gray-900 dark:text-white">
